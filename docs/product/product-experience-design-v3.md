...
#### 1.7 OpenClaw 兼容层（关键约束）

**OpenClaw 运行时深度依赖以下文件，不能废弃，必须保留：**

| 文件 | OpenClaw 依赖方式 | 不能动的原因 |
|------|------------------|-------------|
| USER.md | AGENTS.md RULE-ZERO 每次对话必须 read/edit；Cron 状态读写 | 30+ 处引用，是 OpenClaw 的核心接口 |
| MEMORY.md | compaction 时自动加载，长期记忆锚点 | OpenClaw 内置行为 |
| people/*.md | AGENTS.md 路由读取人物上下文 | Skill 执行时直接 read |
| diary/*.md | 日记记录 + 脚本输入 | 文件系统依赖 |
| SOUL.md / IDENTITY.md | agent 启动时加载 | 只读，不改 |
| HEARTBEAT.md | Heartbeat 触发规则 | OpenClaw 调度依赖 |

**设计原则：memU 是增强层，不是替代层。.md 文件仍然是 OpenClaw 的 single source of truth，memU 提供更深的提取和检索能力。**
...
对话结束时的数据流：
                    ...
                    ┌──→ write USER.md（保持现有 7 字段格式）
                    │
                    ├──→ write people/{name}.md（保持现有 6 维度格式）
Skill 执行          │
（diary/onboarding/ ├──→ write diary/YYYY/MM/DD.md（保持现有六元组格式）
 farewell 等）      │
                    ├──→ write MEMORY.md（长期记忆锚点）
                    │
                    └──→ memu_bridge.py memorize(对话全文)
                         → memU 自动提取到三维 Category
                         → 存入 PostgreSQL
```

**双写意味着：**
1. .md 文件继续按现有格式写入 → OpenClaw 运行时正常工作
2. 同时调 memU memorize() → 获得三维分类 + 自动摘要 + 智能检索
3. Skill 读取时可用 memU retrieve() 获得更丰富的上下文，fallback 到 read .md 文件

**memU 的增量价值（.md 文件做不到的）：**
- 自动三维分类（人物 × 事件 × 自我）
- Category summary 自动聚合（不需要手动维护索引）
- 跨维度检索（"所有和被忽视相关的记忆"——跨人物、跨事件）
- 历史 summary 对比（自我认知变化轨迹）

#### 1.9 数据初始化

现有 .md 文件中的数据需要灌入 memU 作为初始记忆：

| 现有文件 | 灌入方式 |
|---------|---------|
| USER.md | memorize(modality="document") → 提取到 self/* categories |
| people/*.md | 每个文件 memorize(modality="document") → 提取到 people/* categories |
| diary/**/*.md | 每个文件 memorize(modality="document") → 提取到 events/* + people/* |
| memory/*.md | memorize(modality="document") → 提取到 self/行为模式 |

灌入脚本一次性执行。**灌入后 .md 文件继续正常读写，不废弃。**
...
Step 7: 存储（双写）
  → 保留现有写入：write diary/*.md + edit people/*.md + write memory/*.md + edit USER.md
  → 新增：调 memu_bridge.py memorize(本次完整对话内容)
  → memU 自动完成：
...
Step 8: 对话结束处理
  → 保留：edit USER.md + edit people/*.md（OpenClaw 依赖）
  → 新增：memU memorize() 在 Step 7 已覆盖
```

### 4. 六元组记录格式

日记内容仍然由可可在对话中引导生成，继续写入 diary/*.md（OpenClaw 兼容）。同时对话全文作为 Resource 存入 memU，六元组的各项作为 MemoryItem 被提取。
...
Node F+G (建档 + 告别) → 双写：write USER.md + people/*.md（OpenClaw）+ memU memorize()
```
...
| 环节 | v2 | v3 |
|------|------|------|
| 判断是否新用户 | USER.md 不存在 | USER.md 不存在（保持，OpenClaw 依赖）+ memU retrieve() 返回空 |
| 建档 | write USER.md + write people/{name}.md | 保留现有写入 + 新增 memorize(对话) → memU 自动提取 |
| 告别语 | 引用本次对话细节 | 同上（从对话上下文中取） |
...
farewell 帮用户对一段关系做告别——仪式性地封存记忆，提取模式级洞察。双写：.md 文件归档（OpenClaw 兼容）+ memU category 操作。
...
| 环节 | v2 | v3 |
|------|------|------|
| 读取人物档案 | read people/{name}.md | 保留 read people/{name}.md + memU retrieve(category="people/{name}") |
| 提取模式洞察 | 从 people/{name}.md 手动提取 | 保留 + memU get_category_summary("people/{name}") |
| 归档 | person_update(content="[archived]") | 保留 .md 归档 + memU update_category(archived=true) |
| 洞察保存 | user_profile_update(patch="模式级洞察") | 保留写入 USER.md + memU memorize("去名字的模式洞察") |
...
Phase 3: 执行仪式
Phase 4: 归档序列（双写）
  → read people/{name}.md + memU retrieve(category="people/{name}") → 获取完整记忆
  → 提取去名字的模式洞察
  → 写入 USER.md 模式级洞察段 + memU memorize("模式洞察：xxx")
  → people/{name}.md 标记 [archived] + memU update_category(archived=true)
  → 清理 memory/pending_followup.md 相关条目
...
| 文件/目录 | 说明 |
|----------|------|
| USER.md | 继续按现有 7 字段格式读写，同时双写到 memU |
| MEMORY.md | 继续作为 compaction 锚点，同时双写到 memU |
| people/*.md | 继续按现有 6 维度格式读写，同时双写到 memU |
| diary/**/*.md | 继续按现有六元组格式写入，同时双写到 memU |
| memory/*.md | 继续写入（pattern_log、pending_followup 等），同时双写到 memU |
| SOUL.md / IDENTITY.md | 只读，不变 |
| HEARTBEAT.md | 更新触发规则以整合新 skill，格式不变 |

### Skill 重命名/合并

| 旧 Skill | 新 Skill | 处理方式 |
|----------|---------|---------|
| skills/breathing-ground/ | → skills/calm-body/ | 改名升级，旧目录删除 |
| skills/pattern-mirror/ | → skills/see-pattern/ | 合并 growth-story，旧目录删除 |
| skills/growth-story/ | → skills/see-pattern/ | 合并入 see-pattern |
| skills/decision-cooling/ | → skills/face-decision/ | 扩展，旧目录删除 |

### 脚本演进

| 旧脚本 | 处理方式 |
|-------|---------|
| scripts/pattern_engine.py | 保留（OpenClaw skill 仍调用），未来 memU retrieve() 成熟后逐步替代 |
| scripts/growth_tracker.py | 同上 |
| scripts/weekly_review.py | 同上 |
| scripts/emotion_counter.py | 同上 |
| scripts/crisis_detector.py | 逻辑移入 crisis skill，脚本保留兼容 |
