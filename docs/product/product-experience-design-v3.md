...
| 文件 | 加载时机 | 为什么必须保留 |
|------|---------|--------------|
| USER.md | 每次对话启动时自动加载 | OpenClaw 的核心上下文接口，AGENTS.md 30+ 处引用，Cron 状态也在里面 |
| MEMORY.md | compaction 时自动加载 | 长期记忆锚点，不受时间衰减，OpenClaw 内置行为 |
| SOUL.md / IDENTITY.md | agent 启动时加载 | 只读，人格内核，不需要改 |
| HEARTBEAT.md | Heartbeat 触发时加载 | 调度规则，更新内容但保持格式 |

**二次读取文件（Skill 按需读取，可以被 memU 替代）：**

| 文件 | 读取方式 | v3 处理 |
|------|---------|--------|
| people/*.md | Skill 执行时 `read("people/xxx.md")` | **由 memU 替代**，Skill 改为调 memU retrieve() |
| diary/*.md | Skill/脚本按需读取 | **由 memU 替代**，不再写入 |
| memory/*.md | Skill/脚本按需读取 | **由 memU 替代**，不再写入 |
...
对话结束时的数据流：
                    ...
                    ┌──→ write USER.md（首次读取，OpenClaw 必须）
Skill 执行          ├──→ write MEMORY.md（首次读取，compaction 锚点）
                    │
                    └──→ memu_bridge.py memorize(对话全文)
                         → memU 自动提取到三维 Category
                         → 存入 PostgreSQL
                         → 替代 people/*.md + diary/*.md + memory/*.md

对话开始时：
  OpenClaw 自动加载 → USER.md + MEMORY.md → 基础上下文
  Skill 需要上下文 → memU retrieve() → 替代 read people/*.md
```

**memU 的价值（二次读取文件做不到的）：**
- 自动三维分类（人物 × 事件 × 自我）
- Category summary 自动聚合（不需要手动维护 people/*.md 的 6 维度）
- 跨维度检索（"所有和被忽视相关的记忆"——跨人物、跨事件）
- 历史 summary 对比（自我认知变化轨迹）

#### 1.9 数据初始化

现有二次读取文件中的数据需要一次性灌入 memU：

| 现有文件 | 灌入方式 | 灌入后 |
|---------|---------|-------|
| USER.md | memorize(modality="document") → 提取到 self/* categories | 继续写入（OpenClaw 依赖） |
| people/*.md | 每个文件 memorize(modality="document") → 提取到 people/* categories | **不再写入**，由 memU 管理 |
| diary/**/*.md | 每个文件 memorize(modality="document") → 提取到 events/* + people/* | **不再写入**，由 memU 管理 |
| memory/*.md | memorize(modality="document") → 提取到 self/行为模式 | **不再写入**，由 memU 管理 |
...
Step 7: 存储
  → 首次读取文件：edit USER.md（保持 7 字段）+ write MEMORY.md（锚点更新）
  → 二次读取文件不再写入（people/*.md, diary/*.md, memory/*.md 由 memU 管理）
  → 调 memu_bridge.py memorize(本次完整对话内容)
  → memU 自动完成：
...
Step 8: 对话结束处理
  → edit USER.md（首次读取，OpenClaw 依赖）
  → memU memorize() 在 Step 7 已覆盖所有二次读取数据
```

### 4. 六元组记录格式

日记内容仍然由可可在对话中引导生成。对话全文作为 Resource 存入 memU，六元组的各项作为 MemoryItem 被提取到三维 Category 中。不再写入 diary/*.md 文件。
...
Node F+G (建档 + 告别) → write USER.md（首次读取）+ memU memorize()（替代 people/*.md）
```
...
| 环节 | v2 | v3 |
|------|------|------|
| 判断是否新用户 | USER.md 不存在 | USER.md 不存在（首次读取，OpenClaw 依赖） |
| 建档 | write USER.md + write people/{name}.md | write USER.md + memU memorize(对话)（people 由 memU 管理） |
...
farewell 帮用户对一段关系做告别——仪式性地封存记忆，提取模式级洞察。人物数据由 memU 管理，洞察写入 USER.md（首次读取）。
...
| 归档 | person_update(content="[archived]") | memU update_category(archived=true) |
| 洞察保存 | user_profile_update(patch="模式级洞察") | write USER.md 模式级洞察段（首次读取）+ memU memorize() |
...
Phase 3: 执行仪式
Phase 4: 归档序列
  → memU retrieve(category="people/{name}") → 获取完整记忆
  → 提取去名字的模式洞察
  → write USER.md 模式级洞察段（首次读取，OpenClaw 依赖）
  → memU memorize("模式洞察：xxx") → 存入 self/行为模式
  → memU update_category("people/{name}", archived=true)
...
| 文件/目录 | 说明 |
|----------|------|
| people/*.md | 数据灌入 memU 后不再写入，Skill 改用 memU retrieve() |
...
| 旧脚本 | 处理方式 |
|-------|---------|
| scripts/pattern_engine.py | 废弃，由 memU retrieve() 替代 |
| scripts/growth_tracker.py | 废弃，由 memU retrieve() + Category summary 替代 |
| scripts/weekly_review.py | 废弃，由 memU retrieve() 替代 |
| scripts/emotion_counter.py | 废弃，由 memU Category summary 替代 |
| scripts/crisis_detector.py | 逻辑移入 crisis skill |
