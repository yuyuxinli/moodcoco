...
| Skill | 做什么 | 触发条件 | 心理学基础 | 优先级 | 执行模式 |
|-------|--------|---------|-----------|--------|---------|
| **listen** | 情绪承接与倾听。不分析不建议，就是在。 | 用户带着情绪来、需要有人听 | 以人为中心疗法、无条件积极关注 | P0 | Single-turn |
| **untangle** | 帮用户把混乱的事情理清楚。拆开人、事、情绪。 | 用户思绪混乱、多件事缠在一起 | 叙事疗法外化技术、Lazarus 压力评估 | P0 | Single-turn |
| **see-pattern** | 跨对话识别关系模式和行为规律 | 同类对话 3+ 次、memory 有足够数据 | 叙事疗法重写、CBT 模式识别 | P0 | Agentic（需读记忆 + 跨关系分析，说错会伤害用户） |
| **face-decision** | 帮用户面对选择、理清利弊，但不替她决定 | 用户面临选择、犹豫不决 | 动机式访谈、ACT 价值观澄清 | P1 | Agentic（需判断冲动程度 + 调度回访） |
| **know-myself** | 帮用户探索自我认知 | 用户没有具体事件，在想自己 | IFS 部分工作、ACT 自我觉察 | P1 | Agentic（需谨慎措辞，不能贴标签） |
| **crisis** | 危机干预。评估风险等级，提供专业转介 | P0 关键词或 P1 模糊信号累积 | QPR 技术（提问-劝说-转介） | P0（安全底线） | Agentic（必须 pre-check + 多步确认，错误代价极高） |
| **calm-body** | 身体层面即时稳定：呼吸引导、感官着陆、助眠 | 焦虑发作、失眠、恐慌 | 循环叹息（Stanford RCT）、4-7-8、5-4-3-2-1 | P0 | Single-turn（引导性指令，格式固定） |
| **base-communication** | 所有 skill 的地基：承接/澄清/轻推动三组沟通技术 | 始终加载，不单独触发 | 临床心理基本技术 | P0（地基） | Single-turn |

**执行模式说明：**
- 默认 **Agentic**：需要多步推理（读记忆、跨关系分析、风险评估），错误代价高，必须经过 pre-check 和多步确认。
- **Single-turn**：不需要复杂决策，错误代价低，LLM 单轮即可完成。适合纯倾听、固定格式引导、简单承接。
