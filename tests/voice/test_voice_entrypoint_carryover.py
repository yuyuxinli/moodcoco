...
@pytest.mark.asyncio
async def test_bridge_caps_carryover_skills_to_recent_two(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import backend.fast as fast_mod
    import backend.slow as slow_mod
    ...
    assert calls["fast"][1]["deps"].skill_bundle == ["skill-b", "skill-c"]
    assert agent._slow_state["carryover_skills"] == ["skill-b", "skill-c"]
