...
if weekly_recommendations.plan_intensity == "reduced":
  ...
  available_hours = min(available_hours, 2.0)
...
for each subject in exam_subjects:
  # 基础分配 = 可用时间 * 阶段权重 * 提分ROI
  phase_weight = PHASE_WEIGHTS[current_phase][subject.type]
  roi_score = gap_factor * marginal_factor * phase_weight
  base_hours = available_hours * normalize(roi_score)
...
# 若保底修正后总和 > available_hours，从权重最低科目减
...
如果 sum(cap) < retest_line（所有科目 90% 封顶后总分仍不够），在推送中坦诚提示："按当前水平，要达到 {retest_line} 分的目标还有不小差距。先把每科基础打扎实，后面再看要不要调整目标。"
...
| 崩溃恢复（F6 后第二天） | "昨天辛苦了。今天不用多想，就做一件事就好。" |
...
| normal | bad_day 或 tired | easy | 60% |
| normal | done_none | easy | 60% |
| momentum | done_most/done_some 或 done_none | normal | 100% |
| easy | done_all 或 done_most/done_some | normal | 100% |
| easy | 连续 2 天 done_none | minimal | 40%（最多 2h） |
| minimal | 回复了任何内容 | easy | 60% |
| minimal | 连续 2 天无回复 | dormant | 停止推送，F6 崩溃接住介入 |
| dormant | 用户主动发消息 | easy | 60% |

**关键：momentum 模式不加量。** 加码 = 惩罚表现好的人 = 导致放弃。

**F6 崩溃降级优先级：** F6 触发后强制覆盖状态机 → 切为 minimal 并锁定 3 天。锁定期内状态机转换条件挂起。Day 4 自动解除 → easy。用户主动说"准备好了"可提前解除。
...
推送后用户表达危机信号（"不想活了"等）→ 立即中断计划对话，走 AGENTS.md RULE-ZERO 安全流程。
