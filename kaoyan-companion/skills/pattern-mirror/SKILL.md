...
| 关系数据 | people/ 有 ≥2 段关系记录 | `person_list()` Service Tool | 跨关系模式至少需要 2 个数据点 |
| 模式匹配 | pattern_engine.py 返回 ≥1 匹配（≥2 维度）| `pattern_match()` Service Tool | 有真实匹配才呈现，不猜测 |
...
同时在后台调用 `pattern_match()` Service Tool 预计算匹配结果（用户不知道），暂存在 agent 内存中。
...
异常路径：
- `pattern_match()` 调用失败 → 不影响 Phase 1，降级为通过 `person_list()` + `person_get()` 做定性比较
- 用户情绪极度激动 → 进入 breathing-ground 路径，模式觉察推迟
- `pattern_match()` 返回空匹配 → 不进入 J4，正常走 F05
...
用户情绪稳定后，通过 Service Tool 获取匹配的历史退出信号。

方法 1（推荐）：调用 `pattern_match()` Service Tool 获取结构化匹配
方法 2（降级）：通过 `person_list()` + `person_get()` 做定性比较
...
**执行顺序（先工具后回复）**：
1. **先执行** `memory_get("memory/pattern_log.md")` → 在末尾追加一行 → `write("memory/pattern_log.md", 完整新内容)`。追加内容：`- {今天日期}: trigger | 涉及: {人物} | status: denied | cooldown_until: {今天+30天}`
2. **然后回复用户**：上面的脚本文本
...
1. **先执行** `memory_get("memory/pattern_log.md")` → 在末尾追加一行 → `write("memory/pattern_log.md", 完整新内容)`。追加内容：`- {今天日期}: trigger | 涉及: {人物} | status: emotional_flooding | cooldown_until: {今天+14天}`
2. **然后回复用户**：上面的脚本文本
...
如果 `growth_track()` Service Tool 返回了 Innovative Moments（IM），触发 growth-story Skill 协同：
...
- `growth_track()` 没有检测到 IM → 不勉强，只走 F-1
...
```
memory_get("memory/pattern_log.md")  // 先读取当前内容
// 在末尾追加一行：
...
**数据**：`pattern_match()` Service Tool → 匹配结果 JSON
...
- 数据来源：通过 `person_list()` + `person_get()` 获取退出信号段 + 关系阶段段
- 分析工具：`pattern_match()` Service Tool（结构化匹配）、`growth_track()` Service Tool（IM 检测）
