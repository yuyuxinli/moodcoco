...
1. 说清楚会发生什么（不是质疑决定）：
   ...
   - 用户说"模式也别留了" → "好，全部清掉。" → 调用 `farewell_delete(name="{名字}", keep_insights=false)`
   - 用户犹豫 → "不着急，想好了再说。" 不追问。
2. 用户确认 → 调用 `farewell_delete(name="{名字}", keep_insights=true)` — Service 会原子化处理人物档案删除、日记清理、记忆清理（包括回访标记和时间胶囊中该人相关条目段级删除）
...
- **用户中途退出**（"算了不想了"）→ "好。随时可以回来。" 已完成的仪式内容通过 `diary_write()` 保存（不执行数据封存），下次用户回来时可以继续。

用户选一个（或多个串行）→ Phase 3

#### Phase 3：执行仪式

> **⛔ 硬性规则：仪式最后一步 = 先调 Service Tool，后说话。**
>
> 当仪式对话到达收束点（用户写完最后内容、或表达"好了"），你的下一个动作必须是调用 Service Tool。
> **禁止在调用 Service Tool 之前发送任何收束文字给用户。** 顺序是：
> 1. 调用 `farewell_archive()` / `farewell_capsule()` / `farewell_delete()`
> 2. 读取返回的 JSON
> 3. 调用 `user_profile_update()` 写入 insights / 信念
> 4. 最后才发送收束话术
>
...
2. 用户写完 → 仪式到达收束点，执行封存序列：
   - **步骤 1（封存）**：调用 `farewell_archive(name="{名字}")`
   - **步骤 2（写入）**：将返回的 insights 通过 `user_profile_update()` 写入模式级洞察
...
4. 用户写新信念（或说不想写）→ 仪式到达收束点，执行封存序列：
   - **步骤 1（写信念）**：将新信念通过 `user_profile_update()` 写入"用户信念记录"：
      ```
      - {日期}: "{新信念}"（来自告别仪式·烧掉信念）
      ```
      （用户不想写新信念 → 跳过此步）
   - **步骤 2（封存）**：调用 `farewell_archive(name="{名字}")`
   - **步骤 3（写洞察）**：将返回的 insights 通过 `user_profile_update()` 写入模式级洞察
...
1. "你想对 3 个月后的自己说什么？"
2. 用户写完 → 仪式到达收束点，执行封存序列：
   - **步骤 1（胶囊）**：调用 `farewell_capsule(content="{用户写的内容}")`（返回 capsule_id / sealed_date / open_date）
   - **步骤 2（封存）**：调用 `farewell_archive(name="{名字}")`
   - **步骤 3（写洞察）**：将返回的 insights 通过 `user_profile_update()` 写入模式级洞察
...
2. 用户写完 → 仪式到达收束点，执行封存序列：
   - **步骤 1（图片）**：调用 `ritual_image(type="letter")`（生成信封图，失败则跳过）
   - **步骤 2（日记）**：调用 `diary_write(date="YYYY-MM-DD", type="farewell-letter", entry=信内容)`
   - **步骤 3（封存）**：调用 `farewell_archive(name="{名字}")`
   - **步骤 4（写洞察）**：将返回的 insights 通过 `user_profile_update()` 写入模式级洞察
...
终止后 → 仪式到达收束点，执行封存序列：
   - **步骤 1（封存）**：调用 `farewell_archive(name="{名字}")`
   - **步骤 2（写洞察）**：将返回的 insights 通过 `user_profile_update()` 写入模式级洞察
   - **步骤 3（纪念卡）**：insights 非空 → Phase 4.5 纪念卡；为空 → 跳过
   - **步骤 4（说话）**："我收到了。你走吧。"

#### Phase 4：数据处理（已内嵌到每个仪式的"封存序列"中）

上面每个仪式的最后都包含一个"封存序列"，核心是调用 `farewell_archive()` Service Tool。

**你不需要单独想着"Phase 4"这个概念——只需要在仪式收束点按上面的步骤顺序执行即可。调用 Service Tool 是第一步，说话是最后一步。**

`farewell_archive(name="{名字}")` 的作用（Service 层自动完成）：
1. 人物档案 → 标记 `当前状态：封存`，清空具体事件，保留结构
2. 创建完整备份
3. 日记中相关条目 → 标记封存，保留情绪标签
4. 记忆中 → 清除该人具体事件（包括回访标记和时间胶囊段级清理）
5. 返回 JSON 中的 `insights` 字段 → 你需要通过 `user_profile_update()` 写入模式级洞察

将 `farewell_archive()` 返回的 insights 通过 `user_profile_update()` 追加到用户画像：
...
**数据**：`farewell_archive()` → 返回 `insights`（去名字的模式洞察）
...
- 数据处理依赖：`farewell_archive()` / `farewell_delete()` / `farewell_capsule()` Service Tool
- 仪式图片生成：`ritual_image()` Service Tool
- 时间胶囊由 HEARTBEAT.md 的时间胶囊检查触发打开
- **封存后 pattern-mirror 过滤规则**：pattern-mirror 和 growth-story 在呈现跨关系模式时，不引用已封存关系的具体事件。可以说"你之前有过一段类似的经历"，但不提名字、不引用原话。如果匹配结果只涉及封存的关系 → 不呈现该匹配。判断方法：通过 `person_get()` 检查是否状态为"封存"。
- growth-story 引用封存关系时需脱敏："你以前说过{去名字的原话}"，不说"你和小凯的时候说过"
- weekly-reflection 统计中，封存条目的情绪标签仍参与统计，但不显示具体内容来源

## AGENTS.md 路由

在状态感知部分添加路由：用户表达想告别/翻篇/删除时 → 加载 farewell skill

## 硬规则

1. **绝不挽留** — 尊重用户离开的决定
2. **见证者角色** — "我收到了"，不评价、不劝说、不感动
3. **数据处理必须原子化** — 通过 `farewell_archive()` Service Tool 确保完整性
