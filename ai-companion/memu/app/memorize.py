...
class MemorizeMixin:
    ...
    async def _persist_memory_items(
        ...
        *,
        ...
        for (memory_type, summary_text, cat_names), emb in zip(structured_entries, item_embeddings, strict=True):
            ...
            # First try static mapping, then dynamic creation for people/*/events/*
            mapped_cat_ids = self._map_category_names_to_ids(cat_names, ctx)
            unmapped = [n for n in cat_names if n.strip().lower() not in ctx.category_name_to_id]
            for name in unmapped:
                dynamic_id = await self._ensure_dynamic_category(
                    name, ctx, store, client, user
                )
                if dynamic_id and dynamic_id not in mapped_cat_ids:
                    mapped_cat_ids.append(dynamic_id)
    ...
    async def _ensure_dynamic_category(
        self,
        name: str,
        ctx: "Context",
        store: "Database",
        ...
        """Dynamically create a category if it matches people/* or events/* naming.

        Returns the category id if created or already exists, None otherwise.
        """
        key = name.strip().lower()
        existing = ctx.category_name_to_id.get(key)
        if existing:
            return existing

        # Only auto-create for people/* and events/* dimensions
        if not (key.startswith("people/") or key.startswith("events/")):
            return None

        clean_name = name.strip()
        dimension, entity = clean_name.split("/", 1)
        if not entity.strip():
            return None

        if dimension.lower() == "people":
            description = f"用户提到的人物：{entity}。包含与该人物相关的所有互动、事件和关系变化。"
        else:
            description = f"用户的长周期事件：{entity}。包含该事件的进展、关键节点和相关人物。"

        embed_text = f"{clean_name}: {description}"
        vecs = await embed_client.embed([embed_text])
        cat = store.memory_category_repo.get_or_create_category(
            name=clean_name, description=description, embedding=vecs[0], user_data=dict(user or {})
        )
        ctx.category_ids.append(cat.id)
        ctx.category_name_to_id[key] = cat.id
        # Also update the category prompt string for subsequent extractions
        from memu.app.settings import CategoryConfig
        new_cfg = CategoryConfig(name=clean_name, description=description)
        self.category_configs.append(new_cfg)
        self.category_config_map[clean_name] = new_cfg
        self._category_prompt_str = self._format_categories_for_prompt(self.category_configs)

        logger.info("Dynamic category created: %s (id=%s)", clean_name, cat.id)
        return cat.id
