---
name: farewell
description: 告别仪式——仪式性封存记忆，提取模式级洞察，对接 memU 记忆引擎。触发词：跟这段关系说再见、我想翻篇了、把他的东西删了、封存、告别、时间胶囊。
---

# Farewell — 告别仪式（v3 memU）
...
v3 将文件读写替换为 memU 记忆引擎操作：

| 旧方式（v2 文件） | 新方式（v3 memU） |
|---|---|
| `read("people/{name}.md")` 读关系历史 | `exec python3 scripts/memu_bridge.py retrieve --query "{name} 关系历史" --user-id coco_user` |
| `farewell_archive()` Service Tool | 归档序列（见下方 5 步） |
| `farewell_delete()` Service Tool | memU 删除序列（见下方） |
| `farewell_capsule()` Service Tool | `exec python3 scripts/memu_bridge.py memorize` 写入时间胶囊 category |
| `write("people/{name}.md")` 标记封存 | `exec python3 scripts/memu_bridge.py memorize` 写入 archived category |
| `edit("USER.md")` 写洞察 | `write("USER.md")` 模式级洞察段（首次读取文件，保留写入） |
...
```
1. exec python3 scripts/memu_bridge.py retrieve --query "{名字}" --user-id coco_user
   → 获取所有该人相关记忆
2. 从 retrieve 结果中提取模式级洞察（如 keep_insights=true）
   → 去掉名字，替换为"对方"
3. 如 keep_insights=true：
   → write("USER.md") 追加模式级洞察段
   → exec python3 scripts/memu_bridge.py memorize 存入 self/行为模式 category
4. 告知 memU 标记该人 category 为 deleted：
   → exec python3 scripts/memu_bridge.py memorize --input <标记文件>
   （标记文件内容："{名字}的所有记忆已删除，category: people/{名字}, status: deleted"）
```
...
- **用户中途退出**（"算了不想了"）→ "好。随时可以回来。" 已完成的仪式内容通过 memU memorize 保存（不执行归档序列），下次用户回来时可以继续
...
4. 用户写新信念（或说不想写）→ 执行归档序列
   - 新信念写入 USER.md 信念记录：`- {日期}: "{新信念}"（来自告别仪式·烧掉信念）`
   - 用户不想写新信念 → 跳过信念写入
...
这是 v3 farewell 的核心数据操作流程。**每一步都不可省略，顺序不可调换。**

```
步骤 1: memU retrieve — 获取该人所有记忆
  exec python3 scripts/memu_bridge.py retrieve --query "{名字} 关系 模式 洞察" --user-id coco_user
  → 返回 JSON，包含该人相关的所有记忆条目
...
步骤 2: 提取模式级洞察 + 去名字
  ...
  - 将 "{名字}" 替换为 "对方"
  - 将 "和{名字}" 替换为 "和对方"
  - 将昵称变体（阿X、小X、XX哥/姐）也替换
  - 不仅替换目标人名，跨关系匹配中出现的其他人名也替换

步骤 3: write USER.md — 写入模式级洞察段
  read("USER.md") → 在末尾追加：
  ```
  ## 模式级洞察（来自已封存的关系）
  - {去名字的洞察 1}
  ...
  封存日期：{YYYY-MM-DD}
  ```
  （如果已有该段落，追加新条目，不覆盖旧洞察）

步骤 4: memU memorize — 存入 self/行为模式 category
  将去名字的洞察写入临时文件，然后：
  exec python3 scripts/memu_bridge.py memorize --input <临时文件> --user-id coco_user
  memU 会自动归类到 self/行为模式 category

步骤 5: memU 标记 category 为 archived
  将封存标记写入临时文件（内容："{名字}的记忆已封存。category: people/{名字}, status: archived, date: {YYYY-MM-DD}"），然后：
  exec python3 scripts/memu_bridge.py memorize --input <标记文件> --user-id coco_user
  memU 会将该条目存入 people/{名字} category，后续 retrieve 时可通过 status 字段过滤
```

**归档序列失败处理**：
- 步骤 1 retrieve 返回空 → 洞察为空，跳过步骤 2-4，仅执行步骤 3（写入"无可提取洞察"）和步骤 5
- 步骤 4 memorize 失败（API 错误）→ 步骤 3 的 USER.md 写入已完成，洞察不会丢失。重试一次，仍失败则记录错误继续
- 步骤 5 标记失败 → 记录错误，告知用户封存已完成但系统标记待补。不阻塞收束
...
将用户写的内容 + 元数据写入临时文件：
  "时间胶囊内容：{用户写的内容}\n封存日期：{YYYY-MM-DD}\n开启日期：{YYYY-MM-DD + 3个月}\n状态：sealed"
exec python3 scripts/memu_bridge.py memorize --input <胶囊文件> --user-id coco_user
→ memU 自动归类到 events/时间胶囊 category
→ HEARTBEAT.md 的时间胶囊检查负责到期打开
```
...
| 场景 | 处理 |
|------|------|
| memU retrieve 命中已封存人物的记忆 | 检查是否有 `status: archived` 标记 → 不引用具体事件，只引用 USER.md 去名字洞察 |
| 用户主动提起已封存的人 | 承认记得但不引用具体事件。若用户想重新打开 → 新建 memU 条目 |
| see-pattern 匹配已封存关系 | 不引用具体事件，可说"你之前有过一段类似的经历" |
| weekly-reflection 统计 | 封存条目的情绪标签仍参与统计，但不显示具体内容来源 |
...
| diary | 仪式内容（信/信念）可通过 memU memorize 保存 |
...
3. **归档序列 5 步零遗漏** — retrieve → 提取洞察 → 去名字 → 写 USER.md + memU memorize → memU 标记 archived
