...
2. 用户确认 → 用 exec 调用 `archive_manager.py delete`
   - delete 会清理：people/*.md 删除、diary/ 清理、memory/ 清理（包括 pending_followup.md 和 time_capsules.md 中该人相关条目）
...
4. 用户写新信念 → "'{新信念}' 收到了。旧的烧掉了，新的我替你记着。"
   **→ 将新信念写入 USER.md "用户信念记录" 段：**
   ```markdown
   ## 用户信念记录
   - {日期}: "{新信念}"（来自告别仪式·烧掉信念）
   ```
   用 `edit` 工具追加到 USER.md。如果该段不存在则创建。
   这样可可以后遇到类似情境时可以引用这个新信念。
- 用户不想写新信念 → "不急，新的信念不一定要现在就有。旧的烧掉就够了。" → 跳过。
...
2. 用户写完 → "我收到了。这封信我替你保管。"
   → exec ritual_image.py --type letter（生成信封图，失败则跳过）
   → 信内容记录到 diary/YYYY/MM/YYYY-MM-DD.md，标记为"告别信"
