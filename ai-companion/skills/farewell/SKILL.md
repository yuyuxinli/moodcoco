...
1. 说清楚会发生什么（不是质疑决定）：
   ...
   - 用户说"模式也别留了" → "好，全部清掉。" → delete + 手动清除 USER.md 相关模式洞察
   - 用户犹豫 → "不着急，想好了再说。" 不追问。
2. 用户确认 → 用 exec 调用 `archive_manager.py delete`：
   ```bash
   exec python3 skills/farewell/scripts/archive_manager.py delete --person {名字} --people-dir people/ --diary-dir diary/ --memory-dir memory/
   ```
   - **必须用 exec 调用脚本，不要手动 edit/write 文件** — 脚本会原子化处理 people/*.md 删除、diary/ section 级清理、memory/ 清理（包括 pending_followup.md 和 time_capsules.md 中该人相关条目段级删除）
   - 手动操作容易漏掉 pending_followup.md / time_capsules.md 的段级清理
...
4. `memory/` 中 → 清除该人具体事件的记忆（包括 pending_followup.md / time_capsules.md 段级清理）
5. 返回 JSON 中的 `insights` 字段 → 你需要用 edit 工具写入 `USER.md`
