...
进入 Phase 1 前，**必须**先 `user_profile_get()` 读取 `## 模式日志` 区块检查是否满足频率限制。不满足 → 不触发此 skill。
...
2. `user_profile_update()` → 反复出现的模式字段
3. `person_update(name="{名字}", patch="...")` → 跨关系匹配段
4. `diary_write(date="YYYY-MM-DD", ...)` → 对话要点
5. `user_profile_update(patch="## 待回访\n...")` → 如果约定了下次行动
...
每次模式呈现后**必须**在用户档案 `## 模式日志` 区块追加一条记录（通过 `user_profile_update(patch="...")`）：
