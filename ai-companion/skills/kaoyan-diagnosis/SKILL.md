...
诊断依赖 knowledge-base 中的考研数据。执行前用 read 工具加载：

```
read knowledge-base/kaoyan/aliases.yml
read knowledge-base/kaoyan/national-lines.yml
read knowledge-base/kaoyan/schools/{school_code}.yml   # 匹配到院校后读取
```
...
**处理：**
1. 别名/简称归一化（查 `aliases.yml` 的 `schools` 段）
2. 模糊匹配 knowledge-base 院校数据
3. 匹配成功 → ai_message 确认全称："华东师范大学，对吧？"
4. 匹配失败 → ai_message："没找到这个学校，你再确认一下名字？或者说全称试试"
5. 多个候选（如"交大"→ 上海交大/西安交大/北京交大） → ai_options 列出让用户选
6. 歧义处理：查 `aliases.yml` 的 `ambiguous` 段，命中则用 ai_options 让用户选
...
kb_data = {
  ...
  subject_ref_scores: { [code]: number } | null
}
```
...
如果有 `subject_ref_scores` → 直接用作各科目标分。
...
calibration_note（仅在实际修正了分数时显示）：
- 有 CET 校准："（英语预估参考了你的{cet_level}成绩）"
- 有跨考校准："（专业课预估考虑了跨考因素）"
- 有二战校准："（参考了你上次初试 {prev_score} 分的成绩）"
...
**第 4 段不给任何"怎么补"的方向建议、阶段划分或分数目标。诊断只揭示缺口，不给解法。**
...
诊断完成后，用 write 工具将以下字段写入 USER.md 的 `## 考研备考（2027 考研）` 段落：
