#!/usr/bin/env python3
"""
S1: 陌生人 — 首次接触 (v8) — P0: AGENTS.md RULE-ZERO 强制写入 + P1: 中置信度反例
根因分析（v8 修复项）：
1. P0（方向 B）：AGENTS.md 头部注入 RULE-ZERO 强制写入规则
   - 在"# 行为规则"标题后、"## 0. 安全前置检查"之前插入高优先级块
   - 规则包含：告别词触发清单、write/edit 顺序要求（先写后回）、典型场景示例
   - 解决 v4-v7 连续 4 次告别场景跳过 tool call 的问题
2. P1：中置信度分叉试探加反例
   - 在"中置信度"段落后追加 ❌ 错误示例（时态确认/事件追问）
   - 追加 ✅ 正确示例（二选一情绪试探用省略号）
   - 解决 Turn 1 "好烦"触发时态确认而非情绪分叉的问题
3. v7 已有修复保留：
...
async def run_s1():
    ...
    print("[S1v8] Step 1: POST /api/auth/guest/session")
    ...
    print("[S1v8] 预备: POST /api/chat/sessions 创建 MOOD 会话")
    ...
    print(f"[S1v8] Step 2: 连接 Socket.IO (session={session_id}, type=mood)")
    ...
    @sio.event
    async def connect_error(data):  # type: ignore
        print(f"  FAIL connect_error: {data}")

    @sio.event
    async def disconnect():  # type: ignore
    ...
    async def _on_processing_start(_data):
    ...
    # ── Step 4: Turn 2 ────────────────────────────────────────────────────────
    print("[S1v8] Step 4: Turn 2 - 被说太敏感")
    ...
    print("[S1v8] Step 5: Turn 3-5 - 快速连发 (< 1s interval)")
    ...
    print("[S1v8] 等待 workspace 写入 (10s)...")
    ...
    print("[S1v8] Step 8: GET /api/about/self")
    ...
    print("[S1v8] Step 9: GET /api/about/relations")
    ...
    print("[S1v8] Step 10: GET /api/about/relations/小白")
    ...
    print(f"[S1v8] Workspace 检查 (runtime: {RUNTIME_WORKSPACE}):")
...
if __name__ == "__main__":
    ...
    out_path = "/Users/jianghongwei/Documents/moodcoco/.evolve/b_output/S1_raw_v8.json"
