...
def generate_burn(output_path: str, text: str | None = None) -> str:
    ...
    for y_offset in range(400, 600, 30):
        draw.line(
            [(450, y_offset), (630, y_offset)], fill=(210, 195, 170, 100), width=1
        )
...
def generate_capsule(
    ...
    draw.rounded_rectangle(
        [env_left, env_top, env_right, env_bottom], radius=16, fill=(255, 245, 235, 255)
    )
    draw.rounded_rectangle(
    ...
    draw.ellipse(
        [seal_cx - 30, seal_cy - 30, seal_cx + 30, seal_cy + 30], fill=CORAL_PINK
    )
...
def generate_letter(output_path: str, text: str | None = None) -> str:
    ...
    for y in range(280, 760, 40):
        draw.line(
            [(paper_left + 40, y), (paper_right - 40, y)], fill=(235, 225, 210), width=1
        )
    ...
    draw.ellipse(
        [flower_x - 6, flower_y - 6, flower_x + 6, flower_y + 6], fill=WARM_APRICOT
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--type", required=True, choices=["burn", "capsule", "letter"], help="仪式类型"
    )
    parser.add_argument(
        "--output",
        default=None,
    ...
    parser.add_argument("--text", default=None, help="图片上叠加的自定义文字")
