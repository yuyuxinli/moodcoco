...
5. **演化优先用户数据**：有历史线索时，用用户自己的人和事替代预设场景。
...
1. 参考 `MEMORY.md` 和 Slow guidance 里的高频人物 / 事件线索
...
- 新 session 开始时，优先参考 `MEMORY.md` 和 Slow guidance 里的高频人物、近期事件、未完结话题
...
- 只有当某个场景偏好已经稳定到会影响后续推荐时，才由 Slow 用 `write_memory()` 补一个简短锚点
